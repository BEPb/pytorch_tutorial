"""
Python 3.10 программа для изучения рекурентной нейронной сети Pytorch
Название файла 03_recurent.py
...
# Рекуррентная нейронная сеть (многие к одному)
class RNN(nn.Module):
    ...
    def forward(self, x):
        # Установим начальные скрытые состояния и состояния ячеек
        h0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(device) 
        c0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(device)
        
        # Вперед распространять LSTM
        out, _ = self.lstm(x, (h0, c0))  # out: размер тензора (batch_size, seq_length, hidden_size)
